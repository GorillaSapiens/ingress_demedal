...
foreach $file (`ls output`) {
   $file =~ s/[\x0a\x0d]//g;
   `convert output/$file -resize "$scalescale!" RGB:compare/b/$file`;
}
...
foreach $file (`ls references`) {
   ...
   if (!($file =~ /\.pl/)) {
      ...
      if ($h != 0 && $w != 0) {
         $base = $file;
         $base =~ s/\..*//g;
         `convert references/$file -background black -alpha remove -alpha off -trim -resize "$scalescale!" RGB:compare/a/$base.png`
      }
   }
}
